...
import {
  ...
  useGetPostCategoryById,
} from "apicalls/hooks/admin/usePostCategory";
...
const NewCategory = ({ history }) => {
  ...
  // api calls
  const { data: cateData } = useGetPostCategoryById(category_id);
  const { mutateAsync, isSuccess, data, isLoading } = useUpdatePostCategory();
